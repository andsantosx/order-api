...
export function sanitizeUserData(data: {
    ...
    if (data.name !== undefined) {
        sanitized.name = truncate(
            normalizeSpaces(stripHtml(data.name)),
...
export function sanitizeAddressData(data: {
    ...
    if (data.city !== undefined) {
        sanitized.city = truncate(
            normalizeSpaces(stripHtml(data.city)),
            VALIDATION.MAX_ADDRESS_LENGTH
        );
    }
 ...
 * Remove HTML e valida comprimentos
 */
export function sanitizeCategoryData(data: {
    name?: string;
    description?: string;
}): {
    name?: string;
...
export function isValidImageUrl(url: string): boolean {
    try {
        ...
        return VALIDATION.ALLOWED_IMAGE_DOMAINS.some(
            (domain: string) => fullUrl.startsWith(domain.toLowerCase())
        );
    } catch {
        return false;
    }
