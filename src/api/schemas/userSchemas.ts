...
export const registerSchema = z.object({
    body: z.object({
        ...
        email: z.string()
            ...
            .trim(),
        password: z.string()
            .min(SECURITY.MIN_PASSWORD_LENGTH, `Senha deve ter no mínimo ${SECURITY.MIN_PASSWORD_LENGTH} caracteres`)
...
export const createAddressSchema = z.object({
    body: z.object({
        street: z.string()
            ...
            .max(VALIDATION.MAX_ADDRESS_LENGTH, `Rua deve ter no máximo ${VALIDATION.MAX_ADDRESS_LENGTH} caracteres`),
        city: z.string()
            .min(VALIDATION.MIN_CITY_LENGTH, `Cidade deve ter no mínimo ${VALIDATION.MIN_CITY_LENGTH} caracteres`)
            .max(VALIDATION.MAX_ADDRESS_LENGTH, `Cidade deve ter no máximo ${VALIDATION.MAX_ADDRESS_LENGTH} caracteres`),
        state: z.string()
            .length(VALIDATION.STATE_LENGTH, `Estado deve ter ${VALIDATION.STATE_LENGTH} caracteres (ex: SP, RJ)`)
            .toUpperCase(),
        zipCode: z.string()
        ...
        country: z.string()
            ...
            .max(VALIDATION.MAX_ADDRESS_LENGTH, `País deve ter no máximo ${VALIDATION.MAX_ADDRESS_LENGTH} caracteres`)
            .default('Brasil')
    }),
});
