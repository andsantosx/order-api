...
import { MercadoPagoPaymentStatus, MercadoPagoStatusDetail } from '../../types/domain-enums';
...
export class Payment {
  ...
  async create({ body }: { body: PaymentRequestBody }): Promise<MercadoPagoPaymentResponse> {
    ...
    const statusDetail: MercadoPagoStatusDetail = isError
      ? MercadoPagoStatusDetail.CC_REJECTED_HIGH_RISK
      : isPending
        ? isPix
          ? MercadoPagoStatusDetail.PENDING_WAITING_TRANSFER
          : MercadoPagoStatusDetail.IN_PROCESS
